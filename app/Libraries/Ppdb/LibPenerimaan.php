...
class LibPenerimaan
{
    ...
    static function fetchDataRekapPendaftaran($auth_data, $id_penerimaan)
    {
        ...
        $jurusan = Jurusan::select('jurusan.id_jurusan', 'jurusan.nm_jurusan')
            ->selectRaw("(SELECT COUNT(id_c_siswa) FROM calon_siswa_baru WHERE calon_siswa_baru.id_penerimaan = '" . $id_penerimaan . "' AND calon_siswa_baru.id_pilihan_jurusan_1 = jurusan.id_jurusan AND tgl_submit_form IS NOT NULL AND calon_siswa_baru.deleted_at IS NULL) AS jumlah_p1,
                                (SELECT COUNT(id_c_siswa) FROM calon_siswa_baru WHERE calon_siswa_baru.id_penerimaan = '" . $id_penerimaan . "' AND calon_siswa_baru.id_pilihan_jurusan_2 = jurusan.id_jurusan AND tgl_submit_form IS NOT NULL AND calon_siswa_baru.deleted_at IS NULL) AS jumlah_p2,
                                (SELECT COUNT(id_c_siswa) FROM calon_siswa_baru WHERE calon_siswa_baru.id_penerimaan = '" . $id_penerimaan . "' AND calon_siswa_baru.id_pilihan_jurusan_3 = jurusan.id_jurusan AND tgl_submit_form IS NOT NULL AND calon_siswa_baru.deleted_at IS NULL) AS jumlah_p3,
                                (SELECT COUNT(id_c_siswa) FROM calon_siswa_baru WHERE calon_siswa_baru.id_penerimaan = '" . $id_penerimaan . "' AND calon_siswa_baru.id_pilihan_jurusan_1 = jurusan.id_jurusan AND tgl_submit_form IS NOT NULL AND calon_siswa_baru.deleted_at IS NULL) AS jumlah_submit_form,
                                (SELECT COUNT(id_c_siswa) FROM calon_siswa_baru WHERE calon_siswa_baru.id_penerimaan = '" . $id_penerimaan . "' AND calon_siswa_baru.id_pilihan_jurusan_1 = jurusan.id_jurusan AND tgl_proses_verifikasi IS NOT NULL AND status_verifikasi = 2 AND calon_siswa_baru.deleted_at IS NULL) AS jumlah_antri_verifikasi,
                                (SELECT COUNT(id_c_siswa) FROM calon_siswa_baru WHERE calon_siswa_baru.id_penerimaan = '" . $id_penerimaan . "' AND calon_siswa_baru.id_pilihan_jurusan_1 = jurusan.id_jurusan AND tgl_verifikasi_dokumen IS NOT NULL AND status_verifikasi = 1 AND calon_siswa_baru.deleted_at IS NULL) AS jumlah_verifikasi,
                                (SELECT COUNT(id_c_siswa) FROM calon_siswa_baru JOIN voucher ON voucher.kode_voucher = calon_siswa_baru.kode_voucher AND voucher.id_penerimaan = calon_siswa_baru.id_penerimaan WHERE calon_siswa_baru.id_penerimaan = '" . $id_penerimaan . "' AND calon_siswa_baru.id_pilihan_jurusan_1 = jurusan.id_jurusan AND voucher.tgl_bayar IS NOT NULL AND calon_siswa_baru.deleted_at IS NULL) AS jumlah_bayar")
    ...
    static function fetchDataCalonSiswaPenetapan($auth_data, $id_penerimaan)
    {
        //'jurusan.nm_jurusan'
        /** get all data penerimaan */
        $calon_siswa_baru = CalonSiswaBaru::select('calon_siswa_baru.id_c_siswa', 'calon_siswa_baru.kode_voucher', 'calon_siswa_baru.nm_c_siswa', 'calon_siswa_baru.nomor_hp', 'calon_siswa_sekolah.nm_sekolah_asal')
            ->join('calon_siswa_sekolah', function ($q) {
                $q->on('calon_siswa_sekolah.id_c_siswa', '=', 'calon_siswa_baru.id_c_siswa')
                    ->whereNull('calon_siswa_sekolah.deleted_at');
            })
