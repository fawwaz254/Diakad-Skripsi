    ...
    static function fetchDataKelas($auth_data, $id = null, $siswa_aktif = false, $only_aktif = true)
    {
        ...
        if ($id == null) {
            ...
            $kelas = Kelas::select(
                ...
                'p1.nm_pengguna as nm_sekretaris',
                'ruangan.nm_ruangan',
            ...
            AND siswa.id_kelas = kelas.id_kelas AND siswa.deleted_at IS NULL) AS total_siswa") : DB::raw("(SELECT COUNT(*) FROM siswa WHERE siswa.id_kelas = kelas.id_kelas AND siswa.deleted_at IS NULL) AS total_siswa")
            )
