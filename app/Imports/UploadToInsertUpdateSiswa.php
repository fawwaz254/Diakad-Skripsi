        ...
        foreach ($data as $key => $data_row) {
            ...
            // } else{
                ...
                if ($id_penerimaan == null || $semester_masuk == null || $jenis_kelamin == null || $jalur == null || $kelas == null || $status == null) {
                    ...
                    //generate id
                    $id_siswa 			= $this->auth_data->sekolah_data->prefix . strtotime($this->now) . uniqid();
                    $id_pengguna 		= $this->auth_data->sekolah_data->prefix . strtotime($this->now) . uniqid();
                    $id_c_siswa 		= $this->auth_data->sekolah_data->prefix . strtotime($this->now) . uniqid();
                    $id_admisi 			= $this->auth_data->sekolah_data->prefix . strtotime($this->now) . uniqid();
                    $id_jalur_siswa 	= $this->auth_data->sekolah_data->prefix . strtotime($this->now) . uniqid();
                    $id_log_kelas_siswa = $this->auth_data->sekolah_data->prefix . strtotime($this->now) . uniqid();
                    $arr[] = array(
            ...
            try {
                ...
                foreach ($arr as $data_siswa) {
                    ...
                    if($check_nis_siswa){
                        ...
                        $row_calon_siswa_ortu['id_siswa'] 		= $data_siswa['id_siswa'];
                        $row_calon_siswa_ortu['id_pengguna'] 	= $data_siswa['id_pengguna'];
                        $row_calon_siswa_ortu['id_c_siswa'] 	= $data_siswa['id_c_siswa'];
                        $row_calon_siswa_ortu['id_kelompok_biaya'] 	= null;
                        $row_calon_siswa_ortu['nis_siswa'] 	= $data_siswa['nis_siswa'];
