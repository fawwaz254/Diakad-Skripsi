...
$validator = Validator::make($request->all(), $syarat);
 if($validator->fails()) {
    ...
    public function geteditprofile(Request $request)
    {
        $input = (object) $request->input();
        $auth_data = $input->auth_data;
        $kerja=UnitKerja::select("id_unit_kerja",
        "nm_unit_kerja")->orderBy("nm_unit_kerja")->get();
        
        // mengambil waktu sekarang
        $now = Carbon::now(env('APP_TIMEZONE', ''));
        if($request->segment(3)=="guru"){
            $datapernikahan = Guru::select("id_guru",
            "id_pengguna",
            "id_unit_kerja",
             "jenis_kelamin",
            "tgl_lahir",
            ...
            "nm_pasangan_ptk",
            "nomor_hp",
            "email")->where('id_pengguna','=',$auth_data->pengguna->id_pengguna)->get()->first();
        ...
        $kota = Kota::select("id_kota",
        "id_provinsi",
        "nm_kota")->where('kota.is_aktif','=',1)->get();
        $provinsi = Provinsi::select("id_provinsi",
        "nm_provinsi")->where('provinsi.is_aktif','=',1)->get();
        $agama = Agama::select(    "id_agama",
        "kode_agama",
        "nm_agama")->get();
        return response()->json([
            'status_code' 	=> 200,
