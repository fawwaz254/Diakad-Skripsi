...
class PaketSoalController extends Controller
{
    ...
    public function detailList(Request $request, $question_package_id = 0, $tipe)
    {
        ...
        if ($tipe == 1) {
            $list_data = Soal::where('id_kategori_soal', $paket_soal->id_kategori_soal)->with('pengguna', 'kategori_soal', 'detail_paket_soal.paket_soal.paket_soal_kelas.kelas')->whereNotIn('id_soal', $list_question_selected);
        } else if ($tipe == 0) {
            $list_data = Soal::where('id_kategori_soal', '!=', $paket_soal->id_kategori_soal)->with('pengguna', 'kategori_soal', 'detail_paket_soal.paket_soal.paket_soal_kelas.kelas')->whereNotIn('id_soal', $list_question_selected);
        } else {
    ...
    public function actionDetailAdd(Request $request)
    {
        ...
        if ($input->id_soal != '0') {
            ...
            foreach ($soals as $soal) {
                if ($question_package_detail = DetailPaketSoal::where(['id_paket_soal' => $input->id_paket_soal, 'id_soal' => $soal->id_soal])->first()) {
                } else {
                    $now = Carbon::now();
                    $question_package_detail = new DetailPaketSoal;
                    $question_package_detail->id_detail_paket_soal = $input->auth_data->sekolah_data->prefix . strtotime($now) . uniqid();
