...
class DataSiswaController extends BaseController
{
    ...
    public function actionUpdateSiswa(Request $request, $id)
    {
        ...
        // rules
        $rules = [
            'akte' => 'file|mimes:jpeg,png,jpg|max:2048',
            'kk' => 'file|mimes:jpeg,png,jpg|max:2048',
            'ijazah' => 'file|mimes:jpeg,png,jpg|max:2048',
        ];
                    ...
                    foreach ($uploadedFiles as $key => $file) {
                        if ($file) {
                            try {
                                ...
                                $updateData = [
                                    'file_' . $key => $filename,
                                    'path_file_' . $key => $filePath,
                                ];
                                DB::table('siswa')->where('nis_siswa', $input->nis_siswa)->update($updateData);

                                // Delete old file if exists
                                $oldFilePath = $siswa->{'path_file_' . $key};
                                if ($oldFilePath) {
                                    Storage::disk('spaces')->delete($oldFilePath);
                                }
                            } catch (\Exception $e) {
                                DB::rollback();
                                return [
                                    'status' => 300, // FAILED
                                    'message' => 'Gagal mengunggah file ' . $key . '. Error: ' . $e->getMessage()
                                ];
                            }
                        }
                    }
