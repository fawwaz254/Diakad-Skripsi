...
class RiwayatPelanggaranController extends BaseController
{
    ...
    public function viewRiwayatPelanggaran(Request $request)
    {
        ...
        $siswa = Siswa::where("id_pengguna", $auth_data->pengguna->id_pengguna)
            ->select("id_siswa")
            ->first();
        $pelanggaran = PelanggaranSiswa::where('id_siswa', $siswa->id_siswa)
            ->with("subkategori_pelanggaran")
            ->get();

        $total_poin = 0;
        foreach ($pelanggaran as $p) {
            $total_poin += $p->subkategori_pelanggaran->poin_subkategori_pelanggaran;
        }

        return view('siswa/pelanggaran/riwayat-pelanggaran/view-riwayat-pelanggaran', compact('auth_data', "total_poin"));
    }
