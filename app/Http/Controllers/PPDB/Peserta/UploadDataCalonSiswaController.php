...
class UploadDataCalonSiswaController extends Controller
{
    ...
    //         ->where('voucher.id_penerimaan', $id_penerimaan)
    ...
    public function uploadFileExcel(Request $request, $id_penerimaan)
    {
        ...
        // Ambil daftar voucher yang tersedia dan belum digunakan
        $vouchers = DB::table('voucher')
            ->leftJoin('calon_siswa_baru', 'voucher.kode_voucher', '=', 'calon_siswa_baru.kode_voucher')
            ->where('voucher.id_penerimaan', $id_penerimaan)
            ->whereNull('calon_siswa_baru.kode_voucher')
            ->select('voucher.kode_voucher')
            ->pluck('kode_voucher') // Mengambil hanya daftar kode voucher dalam bentuk array
            ->toArray();
        ...
        if ($request->hasFile('file-excel')) {
            ...
            // Cek apakah jumlah voucher cukup untuk data yang akan diunggah
            if (count($datas) > count($vouchers)) {
                return response()->json([
                    'status' => 'error',
                    'message' => 'Kode voucher tidak mencukupi untuk semua peserta!',
                ], 400);
            }
            ...
            if (count($datas) > 1) {
                foreach ($datas as $index => $data) {
                    ...
                    if ($exsistStudent) {
                        return response()->json([
                            'status' => 'error',
                            'message' => 'NISN ' . $data['nisn'] . ' telah terdaftar!',
                        ], 500);
                    }

                    // Ambil satu kode voucher untuk siswa ini dan hapus dari daftar
                    $kode_voucher = array_shift($vouchers);
                    ...
                    $siswa->id_c_siswa = Uuid::uuid4()->toString();
                    $siswa->id_penerimaan = $id_penerimaan;
                    $siswa->kode_voucher = $kode_voucher;
                    $siswa->password = null;
                    $siswa->nm_c_siswa = $data['nama_lengkap'] ?? null;
                    $siswa->nm_panggilan = null;
                    $siswa->nik_siswa = $data['nik'] ?? null;
                    ...
                    $siswa->created_by = Auth::user()->id_pengguna;
                    $siswa->updated_by = Auth::user()->id_pengguna;
                    $siswa->nomor_hp = $data['no_telepon_orang_tua'] ?? null;
                    ...
                    $jurusan1 = Jurusan::where('kode_jurusan', $data['kode_pilihan_jurusan_1'])->first();
                    $jurusan2 = Jurusan::where('kode_jurusan', $data['kode_pilihan_jurusan_2'])->first();
                    $jurusan3 = Jurusan::where('kode_jurusan', $data['kode_pilihan_jurusan_3'])->first();

                    $siswa->id_pilihan_jurusan_1 = $jurusan1->id_jurusan ?? null;
                    $siswa->id_pilihan_jurusan_2 = $jurusan2->id_jurusan ?? null;
                    $siswa->id_pilihan_jurusan_3 = $jurusan3->id_jurusan ?? null;
                ...
                return response()->json([
                    'status' => "OK",
                    'message' => "Berhasil Mengunggah Data Calon Siswa!",
                ], 201);
            } else {
                return response()->json([
                    'status' => "Error",
            ...
            return response()->json([
                "status" => "Error",
                "message" => "File Excel Belum Diunggah!"
            ], 400);
        }
    }
}
