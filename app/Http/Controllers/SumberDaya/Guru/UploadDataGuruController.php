...
use App\Imports\UploadToInsertUpdateGuru;
...
class UploadDataGuruController extends BaseController
{
    ...
    public function uploadFileExcel(Request $request)
    {
        ...
        if ($request->hasFile('file-excel')) {
            ...
            $data = (new UploadToInsertUpdateGuru)->toCollection($request->file('file-excel'));
            // $data = Excel::load($path)->get();

            foreach ($data[0] as $value) { // Worksheet 1

                // foreach ($data as $key => $value) {
                $guru = Guru::where('nip_guru', '=', $value['nip'])->first();
                if ($guru == null) {
