...
class SppController extends BaseController
{
    ...
    public function viewMenuPembayaran(Request $request, $tahun_akademik_semester = null, $id_kelas = null, $waktu = null)
    {
        ...
        if (empty($tahun_akademik_semester)) {
            $semester_aktif = Semester::select('thn_akademik_semester')->where('is_aktif_semester', 1)->first();
            $tahun_akademik_semester = $semester_aktif->thn_akademik_semester;
        }

        $data_semester = Semester::select('thn_akademik_semester', 'tahun_ajaran')->orderBy('tahun_ajaran', 'asc')
            ->distinct()
            ->get();
        ...
        if (!empty($id_kelas) && !empty($tahun_akademik_semester)) {
            ...
            $data_siswa = Siswa::with('pengguna', 'pengguna.status_pengguna')
                ->whereIn('id_siswa', $all_data_tagihan->unique('id_siswa')->pluck('id_siswa')->values()->all())
