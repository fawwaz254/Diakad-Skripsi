...
{
    // public function viewTendik(Request $request, $id_role = null)
    // {
    //     # code..
    ...
    //         ->distinct()
    //         ->setBindings([$auth_data->pengguna->id_sekolah, 1, $auth_data->pengguna->id_sekolah])
    //         ->get();

    //     return view('administrator/pengelolaan-akun/tendik/view-tendik', compact('auth_data', 'data_role', 'id_role'));
    // }

    public function actionViewTendik(Request $request)
    {
        # code...
        $input = (object) $request->input();
        $auth_data = auth_data();
        ...
        if ($validator->fails()) {
            ...
            return [
                ...
                'path' => 'pengelolaan-akun/tendik/view-detail/' . $input->id_role
            ];
        }
    }
    public function viewDetailTendik(Request $request, $id_role = 15)
    {
        # code...
        $input = (object) $request->input();
        $auth_data = auth_data();
    ...
    public function datatablesTendik(Request $request, $id_role)
    {
        ...
        $rolePengguna = RolePengguna::select('pengguna.id_pengguna', 'staff.nip_staff', 'pengguna.username', 'pengguna.nm_pengguna', 'pengguna.gelar_depan', 'pengguna.gelar_belakang', 'role.nm_role', 'unit_kerja.nm_unit_kerja')
            ...
            ->join('staff', function ($q) {
                $q->on('staff.id_pengguna', '=', 'pengguna.id_pengguna')
                    ->whereNull('staff.deleted_at');
            })
            ->join('unit_kerja', function ($q) {
                $q->on('unit_kerja.id_unit_kerja', '=', 'staff.id_unit_kerja')
                    ->whereNull('unit_kerja.deleted_at');
            })
            ->where('pengguna.status_join_table', '=', 1)
