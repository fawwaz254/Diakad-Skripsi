...
class CetakPresensiUASController extends BaseController
{
    //
    public function viewCetakPresensiUAS(Request $request)
    {
        # code...
        ...
        return view('akademik/presensi/cetak-presensi-uas/view-cetak-presensi-uas', compact('auth_data', 'data_semester'));
    }

    public function actionViewCetakPresensiUAS(Request $request)
    {
        # code...
        $input = (object) $request->input();
        $auth_data = auth_data();
        ...
        if ($validator->fails()) {
            ...
            return [
                ...
                'path' => 'presensi/cetak-presensi-uas/view-semester-cetak-presensi-uas/'.$input->id_semester
            ];
        }
    }

    public function viewSemesterCetakPresensiUAS(Request $request, $id)
    {
        # code...
        $input = (object) $request->input();
        $auth_data = auth_data();
        ...
        return view('akademik/presensi/cetak-presensi-uas/view-semester-cetak-presensi-uas', compact('auth_data', 'semester', 'id', 'kelas_mp'));
    }

    public function datatablesCetakPresensiUAS(Request $request, $id)
    {
        $input = (object) $request->input();
        $auth_data = auth_data();
    ...
    public function printCetakPresensiUAS(Request $request, $id_ujian_mp, $id_guru)
    {
        # code...
        $input = (object) $request->input();
        $auth_data = auth_data();
        ...
        $data_kelas = LibGuru::fetchDataJadwalUAS($auth_data, $guru->id_pengguna, $semester_aktif->id_semester, 0, $id_ujian_mp);
        ...
        $pdf = PDF::loadView('akademik/presensi/cetak-presensi-uas/download-cetak-presensi-uas', compact('data_siswa', 'auth_data', 'semester_aktif', 'data_kelas'))->setPaper('a4', 'portrait');
        return $pdf->stream();
    }
}
