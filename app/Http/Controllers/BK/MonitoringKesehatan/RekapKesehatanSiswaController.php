...
namespace App\Http\Controllers\BK\MonitoringKesehatan;
...
class RekapKesehatanSiswaController extends BaseController
{
    public function viewRekapKesehatanSiswa(Request $request)
    {
        # code...
        ...
        return view('bk/monitoring-kesehatan/rekap-kesehatan-siswa/view-rekap-kesehatan-siswa', compact('auth_data','data_kelas'));
    }

    public function viewDetailRekapKesehatanSiswa(Request $request, $id_kelas = '-', $id_bulan = null, $tahun = null){
        $input = (object) $request->input();
        $auth_data = $input->auth_data;
        ...
        return view('bk/monitoring-kesehatan/rekap-kesehatan-siswa/view-detail-rekap-kesehatan-siswa', compact('auth_data', 'dates', 'data_bulan', 'bulan', 'data_kelas', 'data_siswa', 'data_pengisian', 'tahun'));
    }

    public function downloadDetailRekapKesehatanSiswa(Request $request, $id_kelas = '-', $id_bulan = null, $tahun = null){
        $input = (object) $request->input();
        $auth_data = $input->auth_data;
        ...
        return Excel::create('Download Data Rekap Kesehatan Kelas '.$data_kelas->nm_kelas.' Bulan '. $bulan->nm_bulan, function ($excel) use ($auth_data, $dates, $data_bulan, $bulan, $data_kelas, $data_siswa, $data_pengisian) {
            $excel->sheet('New sheet', function ($sheet) use ($auth_data, $dates, $data_bulan, $bulan, $data_kelas, $data_siswa, $data_pengisian) {
                $sheet->loadView('bk/monitoring-kesehatan/rekap-kesehatan-siswa/download-rekap-kesehatan-siswa', compact('auth_data', 'dates', 'data_bulan', 'bulan', 'data_kelas', 'data_siswa', 'data_pengisian'));
            });
        })->download('xls');
    }
