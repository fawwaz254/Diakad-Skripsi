    ...
    <div class="row clearfix">
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <div class="card">
                ...
                <div class="body">
                    <form id="form-validation" method="POST" action="{{url(Request::segment(1).'/'.Request::segment(2).'/post-view-siswa')}}">
                        {{csrf_field()}}
                                    ...
                                    @foreach($data_kelas as $data)
                                        @if($id_kelas != null)
                                            @if($data->id_kelas == $id_kelas)
                                                <option value="{{$data->id_kelas}}" selected >{{$data->nm_kelas}} ({{$data->total_siswa}})</option>
                                            @else
                                                <option value="{{$data->id_kelas}}">{{$data->nm_kelas}} ({{$data->total_siswa}})</option>
                                            @endif
                                        @else
                                            <option value="{{$data->id_kelas}}">{{$data->nm_kelas}} ({{$data->total_siswa}})</option>
                                        @endif
                ...
                <div class="body">
                    ...
                    <div class="row clearfix">
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                            <button class="btn btn-block bg-blue waves-effect" onclick="resetPasswordCollect()"><i class="material-icons">update</i><span>Reset Password Kolektif</span></button>
                        </div>
                    </div>
                </div>
                @endif
