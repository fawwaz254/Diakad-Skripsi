...
class CreatePengambilanMpTable extends Migration {
	...
	public function up()
	{
		Schema::create('pengambilan_mp', function(Blueprint $table)
		{
			...
			$table->decimal('persentase_presensi', 10, 0)->nullable()->comment('akumulasi dari presensi_mp');
			$table->decimal('nilai_bobot', 10, 0)->nullable()->comment('Jam KBM x Nilai Indeks (BELUM TERPAKAI)');
			$table->decimal('nilai_angka_rapor', 10, 0)->nullable()->comment('nilai yg ditampilkan di rapor akhir');
			$table->string('nilai_huruf_rapor', 3)->nullable()->comment('nilai yg ditampilkan di rapor akhir');
			$table->decimal('nilai_angka_keterampilan', 10, 0)->nullable()->comment('nilai yg ditampilkan di rapor akhir');
