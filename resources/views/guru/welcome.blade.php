...
<div class="row">
    @foreach (get_moduls() as $modul)
        ...
        @endphp
        @if (isset($featuremenu) && $featuremenu->is_aktif == 1)
            @if (auth_data()->sekolah_data->nm_singkat_sekolah !== 'smpypm2' && $modul->nm_modul == 'Ketidaksesuaian SOP')
            @else
                <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12 " data-toggle="modal"
                    data-target=".bd-modal-lg-{{ $modul->route }}">
                    <div class="card" style="margin: 1rem;">
                        <div class="body bg-teal" style="text-align: -webkit-center;min-height: 26rem;">
                            <img class="media-object" src="{{ url('media/flaticon/' . $modul->route . '.png') }}"
                                width="64" height="64">
                            <h5>
                                {{ $modul->nm_modul }}
                            </h5>
                ...
                <div class="modal fade bd-modal-lg-{{ $modul->route }}" tabindex="-1" role="dialog"
                    aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <div class="row col-lg-12 d-flex justify-content-center"
                                style="background-color:white;border-radus:1rem;">
                                <div class=" d-flex justify-content-center">
                                    @foreach ($modul->menus as $menu)
                                        {{-- @if ($menu->nm_menu == 'Tracer Alumni' && $detail_wali_kelas == null && $role_aktif !== 19 && $role_aktif !== 12) --}}
                                        {{-- @else --}}
                                        <div id="menu-item-{{ $modul->route }}-{{ $menu->page }}"
                                            class="d-flex mx-auto submenu col-lg-4 col-md-4 col-sm-6 col-xs-12">
                                            <a class="target-link d-flex mx-auto"
