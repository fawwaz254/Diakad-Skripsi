...
<div class="container-fluid">
    <div class="block-header">
        <h2><a class="btn bg-blue waves-effect target-link "
                href="{{ url(Request::segment(1) . '#presensi/rekap-absen-tanpa-jadwal') }}"><i
                ...
                <div class="body">
                    <div class="block-header">
                        <h2><a class="btn bg-blue waves-effect" target="_blank"
                                href="{{ url(Request::segment(1) . '/presensi/rekap-absen-tanpa-jadwal/print/' . $id_kelas_mp) }}"><i
                            ...
                            <tbody>
                                ...
                                @foreach ($data_siswa as $siswa)
                                    <tr>
                                        ...
                                        @foreach ($data_presensi as $presensi_mp)
                                            @php
                                                $rekap_absen[$presensi_mp->pertemuan_ke][
                                                    'total_siswa'
                                                ] = $presensi_mp->presensi_mp_siswa->count();
                                                $rekap_absen[$presensi_mp->pertemuan_ke][
                                                    'total_hadir'
                                                ] = $presensi_mp->presensi_mp_siswa->where('kehadiran', 1)->count();
                                            @endphp
                                            @if ($presensi_mp_siswa = $presensi_mp->presensi_mp_siswa->firstWhere('id_siswa', $siswa->id_siswa))
                                                ...
                                                @endif
                                            @else
                                                <td class="is-center bg-amber">Belum di absen</td>
                                            @endif
                                        @endforeach
                                    </tr>
                                @endforeach
