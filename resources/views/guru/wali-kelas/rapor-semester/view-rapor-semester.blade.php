    ...
    <div class="block-header">
        <h2>
            <a class="btn bg-green "
                href="{{ url(Request::segment(1) . '/' . Request::segment(2) . '/rapor-semester/template-excel-data-tambahan/' . $id_kelas) }}"
            ...
            <a class="btn bg-green waves-effect target-link"
                href="{{ url(Request::segment(1) . '#' . Request::segment(2) . '/rapor-semester/importExcel') }}"><i
                    class="material-icons">cloud_upload</i><span>Import Ekcel</span></a>
            <a class="btn btn-info  waves-effect"
                href="{{ url(Request::segment(1) . '/' . Request::segment(2) . '/' . Request::segment(3) . '/print/' . $id_semester . '/' . $id_kelas) }}"
                target="_blank"><i class="material-icons">picture_as_pdf</i><span>Download Rapor</span></a>
            </a>
        </h2>
    </div>
...
<script type="text/javascript">
    ...
    var datatable_url = base_url + '/' + role_url + '/' + modul_url + '/' +
        'rapor-semester/datatables/' +
        id_semester + '/' + id_kelas;
    // var pdf_url = base_url + '/' + role_url + '/' + modul_url + '/' + 'cetak-rapor/printAkhir/' + id_semester;
    var pdf_url = base_url + '/' + role_url + '/' + modul_url + '/' + 'rapor-semester/printPerSiswa';
    var delete_url = base_url + '/' + role_url + '/' + modul_url + '/' +
        'rapor-semester/action-pengembangan-diri/delete';
