...
<body>
    ...
    @foreach ($list_siswa as $siswa)
        ...
        <div class="page" style="margin: 0 auto; width:70%; padding-top:8rem; text-align:center">
            ...
            <div
                style="font-size: 2rem;font-weight:bold;padding:20px;border:1px solid black;margin-top:10rem;border-radius:20px">
            ...
            <div style="padding:20px;border:1px solid black;margin-top:10rem;border-radius:20px;">
                <div class="yellowtail-regular" style="font-size: 2.5rem;font-weight:bold;color: #2b5f94">
                    {{ $auth_data->sekolah_data->nm_yayasan_sekolah }}</div>
                <div style="font-weight: bolder;font-size:3rem; color: #49872e;text-transform:uppercase;">
                    {{ $auth_data->sekolah_data->nm_sekolah }}</div>
                    ...
                    <td>
                        ...
                        <div class="row">
                            <div class="column" style="text-align: center;">NSS:
                                {{ $auth_data->sekolah_data->nss_sekolah }}</div>
                            <div class="column" style="text-align: center;">NDS: 2005020203</div>
                            <div class="column" style="text-align: center;">NPSN:
                                {{ $auth_data->sekolah_data->npsn_sekolah }}</div>
                        </div>
                    </td>
                </tr>
            </table>
            <div
                style="width: 90%; margin: 0 auto; text-align:center; border: 1px solid black; padding:10px; background-color: #ddf5d5">
                        ...
                        <tr>
                            @if ($key_indikator == 0)
                                <td style="text-align: center"
                                    rowspan="{{ $komponen->indikator_rapor_pendukung->count() }}">
                                    {{ $key_komponen + 1 }}</td>
                                <td style="text-align: center"
                                    rowspan="{{ $komponen->indikator_rapor_pendukung->count() }}">
                                    {{ $semester_kelas }}
                                ...
                                <td rowspan="{{ $komponen->indikator_rapor_pendukung->count() }}">
                                    {{ $komponen->nm_komponen }}</td>
                            @endif
                            <td>{{ $loop->iteration }}. {{ $indikator->nm_indikator }}</td>
                            <td style="text-align: center">
                                {{ $indikator->predikat_rapor_pendukung()->where('id_siswa', $siswa->id_siswa)->first()?->nilai }}
                            </td>
                        </tr>
                    @endforeach
                @endforeach
