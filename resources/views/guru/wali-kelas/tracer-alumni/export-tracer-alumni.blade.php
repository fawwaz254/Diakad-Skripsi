            ...
            <div class="card">
                ...
                <div class="body">
                    <form id="form-validation" method="POST" class="row"
                        action="{{ url(Request::segment(1) . '/' . Request::segment(2)) }}/tracer-alumni">
                        {{ csrf_field() }}
                                    ...
                                    <select class="form-control show-tick" name="id_kelas">
                                        @foreach ($data_kelas as $kelas)
                                            <option value="{{ $kelas->id_kelas }}"
                                                @if ($find_kelas->id_kelas == $kelas->id_kelas) selected @endif>
                                                {{ $kelas->nm_kelas }}
                                            </option>
                                        @endforeach
                        ...
                        <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12">
                            @if (!empty($id_kelas) && !empty($tahun_lulus))
                                <a href="guru/wali-kelas/tracer-alumni/export-alumni/{{ $id_kelas }}/{{ $tahun_lulus }}"
...
<script>
    ...
    var datatable_url = base_url + '/' + role_url + '/' + modul_url + '/tracer-alumni/datatables/' + id_kelas + '/' +
        tahun_lulus;
