<div class="container-fluid">
    <div class="block-header">
        <h2><a class="btn bg-blue waves-effect target-link "
                href="{{ url(Request::segment(1) . '#pembina-ekskul/input-nilai-ekskul') }}"><i
    ...
    @else
        <form id="form-validation" method="POST"
            action="{{ url(Request::segment(1) . '/' . Request::segment(2) . '/input-nilai-ekskul/save') }}">
            {{ csrf_field() }}
                    ...
                    <div class="card">
                        ...
                        <div class="body">
                            <div class="table-responsive">
                                <table
                                    class="table table-bordered table-striped table-hover dataTable display responsive wrap"
                                    id="primary_table">
                                    <thead>
                                        <tr>
                                            <th>No</th>
                                                ...
                                                @foreach ($list_komponen as $komponen)
                                                    <td>
                                                        <input type="text" class="form-control"
                                                            name="nilai{{ $komponen->id_komponen_ekskul }}-{{ $siswa->id_siswa }}"
                                                            id="nilai{{ $komponen->id_komponen_ekskul }}-{{ $siswa->id_siswa }}"
                                                            value="{{ collect($siswa->nilai_ekskul)->where('id_komponen_ekskul', $komponen->id_komponen_ekskul)->first()['nilai_komponen_ekskul'] }}">
                                                    </td>
                                                @endforeach
                                                <td>
...
<div style="margin-top: 10px;">
    <div class="col-xs-6 col-sm-6 col-md-6">
        <a class="btn btn-block bg-green waves-effect target-link"
            href="{{ url(Request::segment(1) . '#pembina-ekskul/input-nilai-ekskul/excel/' . $id_semester . '/' . $id_ekskul) }}"><i
