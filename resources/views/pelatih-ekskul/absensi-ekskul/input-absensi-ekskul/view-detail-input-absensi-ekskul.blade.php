...
<div class="container-fluid">
    <div class="block-header">
        <h2><a class="btn bg-blue waves-effect target-link " href="{{url(Request::segment(1).'#absensi-ekskul/input-absensi-ekskul/'.$semester_aktif->id_semester.'/'.$data_ekskul->id_ekskul)}}"><i class="material-icons">backspace</i><span>Kembali</span></a></h2>
    </div>
    <div class="row clearfix">
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <div class="card">
                <div class="header">
                    <h2>
                        ABSEN HARIAN KELAS {{$data_ekskul->nm_ekskul}} Semester {{$semester_aktif->tahun_ajaran}} {{$semester_aktif->nm_semester}} Bulan {{$bulan->nm_bulan}}</h2>
                    </h2>
                </div>
                <div class="body">
                        ...
                        <table class="table table-bordered table-striped table-hover dataTable display responsive nowrap" style="overflow-x:auto;" id="primary_table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    @foreach($data_presensi as $presensi_ekskul)
                                    <td>
                                        <a class="target-link btn btn-info btn-circle waves-effect waves-circle waves-float" href="{{url(Request::segment(1).'#absensi-ekskul/input-absensi-ekskul/manage/'.$semester_aktif->id_semester.'/'.$data_ekskul->id_ekskul.'/'.$presensi_ekskul->id_presensi_ekskul)}}">
                                            <i class="material-icons">edit</i>
                                        </a> 
                                        <button class="btn btn-danger btn-circle waves-effect waves-circle waves-float" onclick="deleteAbsensiAction('{{url(Request::segment(1).'/absensi-ekskul/input-absensi-ekskul/action/delete')}}', this)" data-id="{{$presensi_ekskul->id_presensi_ekskul}}">
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if(response.status == 200){
                            ...
                            loadContent('absensi-ekskul/input-absensi-ekskul/detail/{{$semester_aktif->id_semester}}/{{$data_ekskul->id_ekskul}}/{{$tahun}}/{{$bulan->id_bulan}}');
