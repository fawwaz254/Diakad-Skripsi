<div class="container-fluid">
    <div class="block-header">
        <h2><a class="btn bg-blue waves-effect target-link "
                href="{{ url(Request::segment(1) . '#' . $auth_data->modul_url . '/' . $auth_data->menu_url . '/' . $data_semester->id_semester . '/' . $data_ekskul->id_ekskul) }}"><i
                    class="material-icons">backspace</i><span>Kembali</span></a>
            <a class="btn bg-green waves-effect target-link "
                href="{{ url(Request::segment(1) . '#' . $auth_data->modul_url . '/' . $auth_data->menu_url . '/excel/' . $data_semester->id_semester . '/' . $data_ekskul->id_ekskul) }}"><i
            ...
            <div class="card">
                ...
                <div class="header">
                    <h2>EKSKUL {{ $data_ekskul->nm_ekskul }}
                        <br>
                        SEMESTER {{ $data_semester->tahun_ajaran }} {{ $data_semester->nm_semester }}
                    </h2>
                </div>
                <div class="body">
                    <form id="form-validation" method="POST"
                        ...
                        @if ($presensi_ekskul)
                            <input type="hidden" name="id_presensi_ekskul"
                                value="{{ $presensi_ekskul->id_presensi_ekskul }}">
                        @else
                            <input type="hidden" name="id_presensi_ekskul">
                        @endif
                        <input type="hidden" name="id_semester" required=""
                            value="{{ $data_semester->id_semester }}">
                                        ...
                                        <labe>Pertemuan pekan ke</label>
                                            @if ($presensi_ekskul)
                                                <input type="number" class="form-control" name="pertemuan_ke"
                                                    ...
                                                    value="{{ $presensi_ekskul->pertemuan_ke }}">
                                            @else
                                                <input type="number" class="form-control" name="pertemuan_ke"
                                                    required="" aria-required="true" aria-invalid="true"
                                                    value="">
                                        ...
                                        <labe>Tanggal dan jam presensi</label>
                                            @if ($presensi_ekskul)
                                                <input type="text" class="datetimepicker form-control"
                                                    name="tgl_entry" required="" aria-required="true"
                                                    aria-invalid="true" value="{{ $presensi_ekskul->tgl_entry }}">
                                            @else
                                                <input type="text" class="datetimepicker form-control"
                                                    name="tgl_entry" required="" aria-required="true"
                                                    aria-invalid="true"
...
<script>
    ...
    let id_ekskul = '{{ $data_ekskul->id_ekskul }}',
        id_semester = '{{ $data_semester->id_semester }}';

    @if ($presensi_ekskul)
        let id_presensi_ekskul = '{{ $presensi_ekskul->id_presensi_ekskul }}';
        let datatable_url = base_url + '/' + role_url + '/' + modul_url + '/' + menu_url + '/datatables-detail/' +
            id_semester + '/' + id_ekskul + '/' + id_presensi_ekskul;
    @else
        let datatable_url = base_url + '/' + role_url + '/' + modul_url + '/' + menu_url + '/datatables-detail/' +
            id_semester + '/' + id_ekskul + '/0';
    @endif
    ...
    let primary_table = $('#primary_table').DataTable({
        ...
        columns: [{
            ...
            },
            {
                ...
                render: function(data) {
                    if (data.status_pengguna.status == 1) {
                        return data.nm_kelas + '<br><input type="hidden" name="id_kelas[]" value="' +
                            data.id_kelas + '" >';
                    } else {
                        return '';
                    }
