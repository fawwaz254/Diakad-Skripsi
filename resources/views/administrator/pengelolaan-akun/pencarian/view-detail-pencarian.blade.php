<div class="container-fluid">
    <div class="block-header">
        <h2><a class="btn bg-blue waves-effect target-link "
                href="{{ url(Request::segment(1) . '#pengelolaan-akun/pencarian/view-detail/' . $username_nama_cari) }}"><i
                ...
                <div class="body">
                    <div class="table-responsive">
                        <form id="form-validation" method="POST"
                            action="{{ url(Request::segment(1) . '/' . Request::segment(2) . '/action-pencarian/edit/' . $id_pengguna) }}">
                            <table
                                ...
                                <tr>
                                    <th colspan="2" style="text-align: center;">
                                        @if (!empty($pengguna->path_foto_pengguna))
                                            <img src="{{ Storage::disk('spaces')->url($pengguna->path_foto_pengguna) }}"
                                            ...
                                            <select class="form-control show-tick" name="id_unit_kerja">
                                                ...
                                                @foreach ($unit_kerja_set as $data)
                                                    @if ($data->id_unit_kerja == $id_unit_kerja)
                                                        <option value="{{ $data->id_unit_kerja }}" selected>
                                                            {{ $data->nm_unit_kerja }} -
                                                            {{ $data->nm_singkatan_unit }}</option>
                                                    @else
                                            ...
                                            <select class="form-control show-tick" name="id_role_pengguna">
                                                ...
                                                @foreach ($role_pengguna_set as $data)
                                                    @if ($data->id_role == $pengguna->id_role)
                                                        <option value="{{ $data->id_role_pengguna }}" selected>
                                                            {{ $data->nm_role }}</option>
                                                    @else
                                                        <option value="{{ $data->id_role_pengguna }}">
                                                            {{ $data->nm_role }}</option>
                                                    @endif
                                                @endforeach
                                            </select>
                                ...
                                <tr>
                                    <td colspan="2">
                                        <input type="hidden" class="form-control" name="username_nama_cari"
                                            required="" aria-required="true" aria-invalid="true"
                                            value="{{ $username_nama_cari }}">
                            ...
                            <tr>
                                <td style="width: 50%">
                                    <form id="form-validation1" method="POST"
                                        action="{{ url(Request::segment(1) . '/' . Request::segment(2) . '/action-pencarian/reset-password/' . $id_pengguna) }}">
                                        <input type="hidden" class="form-control" name="id_pengguna" required=""
                                            aria-required="true" aria-invalid="true" value="{{ $id_pengguna }}">
                                        <button class="btn btn-block bg-blue waves-effect" type="submit"><i
                                                class="material-icons">update</i><span>Reset</span></button>
                                    </form>
                                </td>
                                <td style="width: 50%">
                                    <button class="btn btn-block bg-blue waves-effect"><i
                                            class="material-icons">update</i><span>Reset Via SMS/WA</span></button>
                                </td>
                            </tr>
                        </table>
                    </div>
                    ...
                    <div class="body">
                        ...
                        @if ($pengguna->status_join_table == 1 or $pengguna->status_join_table == 2)
                            ...
                            <div class="row clearfix">
                                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                    <a class="btn btn-block bg-blue waves-effect target-link "
                                        href="{{ url(Request::segment(1) . '#pengelolaan-akun/pencarian/add-role-pengguna/' . $id_pengguna . '/' . $username_nama_cari) }}"><i
                                            class="material-icons">library_add</i><span>Tambah Role</span></a>
                                </div>
                            </div>
                        @endif
                    </div>
