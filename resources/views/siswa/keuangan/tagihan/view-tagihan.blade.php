                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <button class="btn btn-warning button_open_modal"
                                                        ...
                                                        data-keterangan="{{ $pembayaran_trs->keterangan }}"
                                                        type="button" waves-effect><i class="material-icons">share</i>
                                                        <span>Share Link Pembayaran</span></button>
                                                    <a target="_blank"
                                                        href="{{ url('payment/detail/' . $pembayaran_trs->id_pembayaran_trs) }}"><button
...
<div class="card">
    <form id="form-validation" method="POST"
        action="{{ url(Request::segment(1) . '/' . Request::segment(2) . '/' . Request::segment(3) . '/generate') }}">
        {{ csrf_field() }}
        <div class="header">
            <h2>TAGIHAN SISWA</h2>
        </div>
                        ...
                        @foreach ($grup_payment_channel as $name => $data_payment_channel)
                            <optgroup label="{{ $name }}">
                                @foreach ($data_payment_channel as $data)
                                    <option value="{{ $data->payment_code }}">{{ $data->payment_name }}
                                        ({{ $data->payment_description }})</option>
                                @endforeach
                            </optgroup>
                        @endforeach
                    </select>
