<div class="container-fluid">
    <div class="block-header">
        <h2><a class="btn bg-blue waves-effect target-link "
                href="{{ url(Request::segment(1) . '#magang-siswa/approve-siswa-magang') }}"><i
...
<script>
    ...
    var datatable_url = base_url + '/' + role_url + '/' + modul_url + '/' + 'approve-siswa-magang/datatables/' +
        id_periode_magang + '/' + id_rekanan_magang + '/' + nis_nama_siswa;
                    ...
                    if (data.id != null) {
                        if (data.status_magang == 1) {
                            ...
                            return '<button class="btn btn-info btn-circle waves-effect waves-circle waves-float" onclick="pengajuanAction(\'' +
                                approve_url + '\', this)" data-id-siswa="' + data.id_siswa +
                                '" data-id-rekanan-magang="' + id_rekanan_magang +
    ...
    function pengajuanAction(approve_url, element) {
        ...
        swal({
            ...
            if (result) {
                $.ajax({
                    type: "POST",
                    url: approve_url + '/' + item.attr('data-id-pengambilan-magang') + '/' + item.attr(
                            'data-id-siswa') + '/' + item.attr('data-id-periode-magang') + '/' + item
                        .attr('data-id-rekanan-magang'),
