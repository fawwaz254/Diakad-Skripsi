...
<script>
    ...
    var primary_table = $('#primary_table').DataTable({
        ...
        columns: [{
            ...
            {
                data: 'action',
                name: 'poster_lowongan_kerja',
                searchable: false,
                orderable: false,
                render: function(data) {
                    if (data.poster) {
                        if (data.note == 'image') {
                            return `<a href=` + data.poster + ` target="_blank"><img src=` + data
                                .poster + ` width=150></a>`;
                        } else {
                            return '<a class="btn btn-success btn-circle waves-effect waves-circle waves-float" target="_blank" href="' +
                                data.poster + '">' +
