    ...
    <div class="row clearfix">
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <div class="card">
                ...
                <div class="body">
                    <form id="form-upload"
                        method="POST"action="{{ url(Request::segment(1) . '/' . Request::segment(2) . '/' . Request::segment(3) . '/action-jurnal-harian/edit/' . $laporan_kerja_harian_tendik->id_lap_kerha_t) }}">
                        {{ csrf_field() }}
                        ...
                        <div class="row clearfix">
                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                <input type="text" class="datepicker form-control" name="tanggal" required=""
                                    ...
                                    value="{{ $laporan_kerja_harian_tendik->tanggal }}">
                            </div>
                        </div>
                        <h2 class="card-inside-title">
                            Jenis Jurnal Harian
                        ...
                        <div class="row clearfix">
                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                <textarea rows="1" cols="50" class="form-control" name="jenis" required="" aria-required="true"
                                    aria-invalid="true">{{ $laporan_kerja_harian_tendik->jenis }}</textarea>
                            </div>
                        </div>
                                ...
                                <select class="form-control selectpicker show-tick" data-show-subtext="true"
                                    ...
                                    @foreach ($all_siswa as $item)
                                        <option value="{{ $item->id_siswa }}"
                                            @if ($item->id_siswa == $laporan_kerja_harian_tendik->id_siswa) selected @endif>
                        ...
                        <div class="demo-radio-button">
                            <input name="status" type="radio" value="1" id="target_1"
                                @if ($laporan_kerja_harian_tendik->status == '1') checked="checked" @endif />
                            <label for="target_1">Selesai</label>
                            <input name="status" type="radio" value="0" id="target_2"
                                @if ($laporan_kerja_harian_tendik->status == '0') checked="checked" @endif />
