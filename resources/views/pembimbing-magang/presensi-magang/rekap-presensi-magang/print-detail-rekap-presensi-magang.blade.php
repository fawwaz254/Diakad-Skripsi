...
<head>
    ...
    <title>Cetak Rekap Presensi Magang</title>
...
<body>
    <div class="page">
        <table style=" width: 90%; margin: 0 auto;font-weight: bold; margin-top:5px;">
            <tr>
                <td colspan="4">
        ...
        <table style="border-style:hidden; width: 90%; margin: 0 auto;font-weight: bold; margin-top:5px;">
            <tbody>
                <tr style="border-style:hidden;">
                    <td style="border-style:hidden;">Nama: {{ $data_siswa->pengguna->nm_pengguna }}</td>
                    <td style="border-style:hidden;text-align: right;">Kelas:
                        {{ $data_siswa->kelas->nm_kelas }}</td>

                </tr>
                <tr style="border-style:hidden;">
                    <td style="border-style:hidden; ">Rekanan:
                        {{ $pembimbing_magang->rekanan->nm_rekanan_magang }}</td>
                    <td style="border-style:hidden;text-align: right;">Mulai:
                        {{ $pembimbing_magang->periode->tgl_magang_mulai }}</td>
                </tr>
                <tr style="border-style:hidden;">
                    <td style="border-style:hidden;">Pembimbing:
                        {{ $pembimbing_magang->pengguna->nm_pengguna }}</td>
                    <td style="border-style:hidden;text-align: right;">Selesai:
                        {{ $pembimbing_magang->periode->tgl_magang_selesai }}</td>
                </tr>
            </tbody>
        </table>
        ...
        <table cellspacing="0" cellpadding="10" style="width: 90%; margin: 0 auto;">
            ...
            <tbody class="body">
                    ...
                    <td style="text-align: center;">
                        @foreach ($data_presensi as $presensi)
                            @if ($presensi->kehadiran == 1)
                                {{ $presensi->presensiMagang->convertDateFormat('tanggal', 'l, j F Y ') }}
                                <br>
                ...
                <tr>
                    ...
                    <td style="text-align: center;">
                        @foreach ($data_presensi as $presensi)
                            @if ($presensi->kehadiran == 3)
                                {{ $presensi->presensiMagang->convertDateFormat('tanggal', 'l, j F Y ') }}
                                <br>
                                @if (!empty($presensi->presensiMagang->keterangan))
                                    {{ $presensi->presensiMagang->keterangan }}<br>
                                @endif
                            @endif
                        @endforeach
                    </td>
                ...
                <tr>
                    ...
                    <td style="text-align: center;">
                        @foreach ($data_presensi as $presensi)
                            @if ($presensi->kehadiran == 2)
                                {{ $presensi->presensiMagang->convertDateFormat('tanggal', 'l, j F Y ') }}<br>
                                @if (!empty($presensi->presensiMagang->keterangan))
                                    {{ $presensi->presensiMagang->keterangan }}<br>
                                @endif
                            @endif
