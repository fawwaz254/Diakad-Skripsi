             ...
             <div class="card">
                 ...
                 <div class="body">
                     <form id="form-validation" method="POST"
                         action="{{ url(Request::segment(1) . '/' . Request::segment(2) . '/post-view-input-rapb') }}">
                         {{ csrf_field() }}
                         <div class="row clearfix">
                     ...
                     <div class="block-header">
                         <h2><a class="btn bg-blue waves-effect target-link"
                                 href="{{ url(Request::segment(1) . '#rapb/input-rapb/add/' . $id_semester_mulai . '/' . $id_semester_selesai) }}"><i
                                     class="material-icons">note_add</i><span>INPUT RAPB</span></a></h2>
                     </div>
 ...
 <script>
     ...
     var modul_url = 'rapb';
     var datatable_url = base_url + '/' + role_url + '/' + modul_url + '/' + 'input-rapb/datatables/' +
     ...
     var kepala_keuangan_url = base_url + '/' + role_url + '/' + modul_url + '/' +
         'action-apv-rapb/approve-kepala-keuangan';
     var edit_url = role_url + '#' + modul_url + '/' + 'input-rapb/edit/' + id_semester_mulai + '/' +
     id_semester_selesai;
             ...
             },
             {
                 ...
                 render: function(data) {
                     if (data.nm_kepala_unit == null) {
                         return '<button class="btn btn-info btn-circle waves-effect waves-circle waves-float" onclick="kepalaUnitAction(\'' +
                             kepala_unit_url + '\', this)" data-idunitkerja="' + data.id_unit_kerja +
                             '" data-idrapb="' + data.id_rapb + '">' +
                 ...
                 render: function(data) {
                     if (data.nm_kepala_keuangan == null) {
                         return '<button class="btn btn-info btn-circle waves-effect waves-circle waves-float" onclick="kepalaKeuanganAction(\'' +
                             kepala_keuangan_url + '\', this)" data-idrapb="' + data.id_rapb + '">' +
                             '    <i class="material-icons">verified_user</i>' +
                             '</button>';
                     } else {
     ...
     function kepalaUnitAction(kepala_unit_url, element) {
         ...
         swal({
             ...
             if (result) {
                 $.ajax({
                     type: "POST",
                     url: kepala_unit_url + '/' + item.attr('data-idrapb') + '/' + item.attr(
                         'data-idunitkerja'),
