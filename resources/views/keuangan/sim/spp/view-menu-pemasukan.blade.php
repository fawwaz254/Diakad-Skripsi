                            ...
                            <tbody>
                                @foreach ($dates as $date)
                                    <tr>
                                        ...
                                        <td>Rp
                                            {{ number_format(
                                                $data_pemasukan_bulan_ini->whereIn('tagihan_biaya.kelas.tingkat', [1, 7, 10])->where('tagihan_biaya.detail_biaya.id_bulan', '>=', $id_bulan)->filter(function ($item) use ($date) {
                                        ...
                                        <td>Rp
                                            {{ number_format(
                                                $data_pemasukan_bulan_ini->whereIn('tagihan_biaya.kelas.tingkat', [2, 8, 11])->where('tagihan_biaya.detail_biaya.id_bulan', '>=', $id_bulan)->filter(function ($item) use ($date) {
                                                        return false !== stristr($item->tgl_pembayaran, $date->format('Y-m-d'));
                                                    })->sum('besar_pembayaran'),
                                            ) }}
                                        </td>
                                        <td>Rp
                                            {{ number_format(
                                                $data_pemasukan_bulan_ini->whereIn('tagihan_biaya.kelas.tingkat', [3, 9, 12])->where('tagihan_biaya.detail_biaya.id_bulan', '>=', $id_bulan)->filter(function ($item) use ($date) {
                                                        return false !== stristr($item->tgl_pembayaran, $date->format('Y-m-d'));
                                                    })->sum('besar_pembayaran'),
                                            ) }}
                                        </td>
                                        <td>Rp
                                            {{ number_format(
                                                $data_pemasukan_bulan_ini->whereIn('tagihan_biaya.kelas.tingkat', [1, 7, 10])->where('tagihan_biaya.detail_biaya.id_bulan', '<', $id_bulan)->where('tagihan_biaya.detail_biaya.keterangan_biaya', '!=', 'PTS')->filter(function ($item) use ($date) {
                                                        return false !== stristr($item->tgl_pembayaran, $date->format('Y-m-d'));
                                                    })->sum('besar_pembayaran'),
                                            ) }}
                                        </td>
                                        <td>Rp
                                            {{ number_format(
                                                $data_pemasukan_bulan_ini->whereIn('tagihan_biaya.kelas.tingkat', [2, 8, 11])->where('tagihan_biaya.detail_biaya.id_bulan', '<', $id_bulan)->where('tagihan_biaya.detail_biaya.keterangan_biaya', '!=', 'PTS')->filter(function ($item) use ($date) {
                                                        return false !== stristr($item->tgl_pembayaran, $date->format('Y-m-d'));
                                                    })->sum('besar_pembayaran'),
                                            ) }}
                                        </td>
                                        <td>Rp
                                            {{ number_format(
                                                $data_pemasukan_bulan_ini->whereIn('tagihan_biaya.kelas.tingkat', [3, 9, 12])->where('tagihan_biaya.detail_biaya.id_bulan', '<', $id_bulan)->where('tagihan_biaya.detail_biaya.keterangan_biaya', '!=', 'PTS')->filter(function ($item) use ($date) {
                                                        return false !== stristr($item->tgl_pembayaran, $date->format('Y-m-d'));
                                                    })->sum('besar_pembayaran'),
                                            ) }}
                                        </td>
                                        @php
                                            $bulan_ini = $data_pemasukan_bulan_ini
                                            ...
                                            $tunggakan_bulan_lalu = $data_pemasukan_bulan_ini
                                                ->where('tagihan_biaya.detail_biaya.id_bulan', '<', $id_bulan)
                                                ->where('tagihan_biaya.detail_biaya.keterangan_biaya', '!=', 'PTS')
                                                ->filter(function ($item) use ($date) {
                                                    return false !== stristr($item->tgl_pembayaran, $date->format('Y-m-d'));
                                                })
                                                ->sum('besar_pembayaran');
                                        @endphp
                                        <td>Rp
                                            {{ number_format($bulan_ini + $tunggakan_bulan_lalu) }}
                                        </td>
                                    </tr>
                                    <tr>
                                @endforeach
                                ...
                                <td>Rp
                                    {{ number_format($data_pemasukan_bulan_ini->whereIn('tagihan_biaya.kelas.tingkat', [1, 7, 10])->where('tagihan_biaya.detail_biaya.id_bulan', '<', $id_bulan)->where('tagihan_biaya.detail_biaya.id_bulan', '<', $id_bulan)->where('tagihan_biaya.detail_biaya.keterangan_biaya', '!=', 'PTS')->sum('besar_pembayaran')) }}
                                </td>
                                <td>Rp
                                    {{ number_format($data_pemasukan_bulan_ini->whereIn('tagihan_biaya.kelas.tingkat', [2, 8, 11])->where('tagihan_biaya.detail_biaya.id_bulan', '<', $id_bulan)->where('tagihan_biaya.detail_biaya.id_bulan', '<', $id_bulan)->where('tagihan_biaya.detail_biaya.keterangan_biaya', '!=', 'PTS')->sum('besar_pembayaran')) }}
                                </td>
                                <td>Rp
                                    {{ number_format($data_pemasukan_bulan_ini->whereIn('tagihan_biaya.kelas.tingkat', [3, 9, 12])->where('tagihan_biaya.detail_biaya.id_bulan', '<', $id_bulan)->where('tagihan_biaya.detail_biaya.id_bulan', '<', $id_bulan)->where('tagihan_biaya.detail_biaya.keterangan_biaya', '!=', 'PTS')->sum('besar_pembayaran')) }}
                                </td>
                                @php
                                    $bulan_ini = $data_pemasukan_bulan_ini->where('tagihan_biaya.detail_biaya.id_bulan', '>=', $id_bulan)->sum('besar_pembayaran');
                                    $tunggakan_bulan_lalu = $data_pemasukan_bulan_ini
                                        ->where('tagihan_biaya.detail_biaya.id_bulan', '<', $id_bulan)
                                        ->where('tagihan_biaya.detail_biaya.keterangan_biaya', '!=', 'PTS')
                                        ->sum('besar_pembayaran');
                                @endphp
                                <td>Rp {{ number_format($bulan_ini + $tunggakan_bulan_lalu) }}</td>
