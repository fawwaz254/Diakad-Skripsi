    ...
    <div class="page">
        ...
        <table cellspacing="0" cellpadding="10" style="width: 100%;">
            <tr>
                ...
                <td colspan=6>
                    <h1 align="center">LAPORAN PEMBAYARAN SISWA PER TANGGAL<br>
        ...
        <table border="1" cellspacing="0" cellpadding="5" style="width: 100%; font-size:small" class="mb-2">
            ...
            @if (isset($data_laporan['data']))
                @foreach ($data_laporan['data'] as $tgl)
                    <tr>
                                ...
                                <td>
                                    @if (collect($tgl['detail'])->where('id_biaya', $idBiaya)->first())
                                        ...
                                        @if (collect($tgl['detail'])->where('id_biaya', $idBiaya)->first()['potongan_biaya'])
                                            <br>
                                            <small>Potongan :
                                                {{ 'Rp ' .number_format(collect($tgl['detail'])->where('id_biaya', $idBiaya)->first()['potongan_biaya']) }}</small>
                                        @endif
                                    @else
                                        {{ '-' }}
                                    @endif
                        ...
                        <td style="text-align: right;"><b>{{ 'Rp ' . number_format($tgl['total_pembayaran']) }}</b>
                        </td>
                    </tr>
                @endforeach
            @endif
            <tr>
                ...
                <th style="text-align: right;">
                    {{ 'Rp ' . number_format(collect($data_laporan['data'])->sum('total_pembayaran')) }}</th>
            </tr>
        </table>
...
<script>
    ...
    document.title = 'Rekap per Tanggal' + ' - ' + start_date + ' - ' + end_date;
    window.print();
</script>
