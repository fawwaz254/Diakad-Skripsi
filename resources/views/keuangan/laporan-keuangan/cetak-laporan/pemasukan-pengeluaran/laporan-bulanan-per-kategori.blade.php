...
<img style="position: absolute;" src="https://diakad.sgp1.digitaloceanspaces.com/{{$auth_data->sekolah_data->nm_singkat_sekolah}}/global/logo-sekolah" height="65">
    ...
    <table width="100%">
        <tr valign=top>
            <td><center><h4 style="margin: 0;">LAPORAN HARIAN</h4></center></td>
        </tr>
        <tr valign=top>
            <td><center><h4 style="margin: 0;">{{$bulan->nm_bulan}} {{$tahun}}</h4></center></td>
        </tr>
    </table>
    <table class="is-bordered" width="100%" border=1 cellpadding=0.75 cellspacing=0 style="background-color: #ffffff; word-wrap:break-word; table-layout: fixed; width: 1440px;">
        <thead>
            <tr valign=middle >
        ...
        <tbody>
            <tr valign=middle >
                ...
                <td style="background-color: black; color: white;"><center>{{$i}}</center></td>
                @endfor
            </tr>
            <tr valign=middle >
                @for($i=1; $i<=$length_column; $i++)
