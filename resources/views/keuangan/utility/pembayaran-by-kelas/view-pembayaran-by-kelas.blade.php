            ...
            <div class="card is-gap">
                ...
                <div class="body">
                    <form id="form-validation" method="POST"
                        action="{{ url(Request::segment(1) . '/' . Request::segment(2) . '/post-view-pembayaran-by-kelas') }}">
                        {{ csrf_field() }}
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    item.parent('td').replaceWith(
                        '<td style="vertical-align:middle;text-align: center;" class="tdbg-' +
                        response
                        .data.month + '">' + response.data.date +
                        '    <br>' +
                        '    <button class="btn btn-danger btn-circle waves-effect waves-circle waves-float" onclick="deleteActionKhusus(this)" data-id="' +
                        response.data.id + '" style="width: 25px; height: 25px;">' +
            ...
            if (result) {
                $.ajax({
                    ...
                    success: function(response) {
                        vex.dialog.alert(response.message);
                        loadContent(
                            'utility/pembayaran-by-kelas/view-detail/{{ $tahun_akademik_semester }}/{{ $id_kelas }}'
                        );
                    },
                    complete: function() {
                        $('button').removeAttr('disabled', 'disabled');
