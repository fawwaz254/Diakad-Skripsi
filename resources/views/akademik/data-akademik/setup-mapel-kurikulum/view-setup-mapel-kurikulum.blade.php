            ...
            <div class="card">
                ...
                <div class="body">
                    <form id="form-validation" method="POST"
                        action="{{ url(Request::segment(1) . '/' . Request::segment(2) . '/post-cari-kurikulum') }}">
                        {{ csrf_field() }}
                                    ...
                                    @foreach ($kurikulum as $data)
                                        @if ($data->id_kurikulum == $id_kurikulum)
                                            <option value="{{ $data->id_kurikulum }}" selected>
                                                {{ $data->nm_kurikulum . ' - ' . $data->jurusan->nm_jurusan }} (
                                                {{ $data->mapel_count }} mata pelajaran )
                                            </option>
                                        @else
                    ...
                    <div class="body">
                        <div class="block-header">
                            <h2><a class="btn bg-blue waves-effect target-link"
                                    href="{{ url(Request::segment(1) . '#data-akademik/setup-mp-kurikulum/add/' . $id_kurikulum) }}"><i
...
<script>
    ...
    var datatable_url = base_url + '/' + role_url + '/' + modul_url + '/' + 'setup-mp-kurikulum/datatables/' +
        id_kurikulum;
