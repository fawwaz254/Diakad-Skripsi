<div class="container-fluid">
    <div class="block-header">
        <h2><a class="btn bg-blue waves-effect target-link" href="{{url(Request::segment(1).'#setting-kelas/kelas/add')}}"><i class="material-icons">note_add</i><span>Tambah Kelas</span></a></h2>
    </div>
...
responsive: true,
        ...
        columns: [
            ...
            { data: 'keterangan_kelas', name: 'keterangan_kelas' },
            { data: 'nm_sekretaris', name: 'nm_sekretaris',
                render: function(data){
                    if(data.nm_sekretaris == 0){
                        return '<a class="target-link btn btn-info btn-circle waves-effect waves-circle waves-float" href="'+ sekretaris_url + '/' + data.id +'">'+
            ...
            { data: 'nm_ruangan', name: 'nm_ruangan',
                render: function(data){
                    if(data.nm_ruangan == 0){
                        return '<a class="target-link btn btn-info btn-circle waves-effect waves-circle waves-float" href="'+ ruangan_url + '/' + data.id +'">'+
                    '    <i class="material-icons">playlist_add</i>'+
                    '</a>';
                    }
                    else{
