                                 ...
                                 <select class="form-control show-tick" name="id_kelas" id="kelas">
                                     ...
                                     @else
                                         ...
                                         @foreach ($kelas as $kelas)
                                             ...
                                             @else
                                                 <option value="{{ $kelas->id_kelas }}">{{ $kelas->nm_kelas }}
                                                 </option>
                                             @endif
                                         @endforeach
                                     @endif
                                     ...
                                     @else
                                         ...
                                         @foreach ($thn_masuk_siswa_list as $tahun)
                                             @if ($thn_masuk_siswa == $tahun->thn_masuk_siswa)
                                                 <option value="{{ $tahun->thn_masuk_siswa }}" selected>
                                                     {{ $tahun->thn_masuk_siswa }}</option>
                                             @else
                                                 <option value="{{ $tahun->thn_masuk_siswa }}">
                                                     {{ $tahun->thn_masuk_siswa }}</option>
                                     ...
                                     @else
                                         ...
                                         @foreach ($jalur as $jalur)
                                             @if ($jalur->id_jalur == $id_jalur)
                                                 <option value="{{ $jalur->id_jalur }}" selected>
                                                     {{ $jalur->nm_jalur }}</option>
                                             @else
                                     ...
                                     @else
                                         ...
                                         @foreach ($status_pengguna as $status_siswa)
                                             @if ($status_siswa->id_status_pengguna == $id_status_pengguna)
                                                 <option value="{{ $status_siswa->id_status_pengguna }}" selected>
                                                     {{ $status_siswa->nm_status_pengguna }}</option>
                                             @else
                                                 <option value="{{ $status_siswa->id_status_pengguna }}">
                                                     {{ $status_siswa->nm_status_pengguna }}</option>
                         ...
                         <div class="row clearfix">
                             <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                 <a class="btn btn-block bg-btn-submit waves-effect target-link"
                                     href="{{ url(Request::segment(1) . '#siswa/update-foto/batch') }}"><i
 ...
 <script>
     ...
     var datatable_url = base_url + '/' + role_url + '/' + modul_url + '/' + 'update-foto/datatables/' + id_jurusan +
