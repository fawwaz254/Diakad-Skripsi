...
<body>
<div class="bg">
  <img class="logo" src="https://diakad.sgp1.digitaloceanspaces.com/{{$auth_data->sekolah_data->nm_singkat_sekolah}}/global/logo-sekolah" alt="Logo Sekolah" style="height:100px; width:90px" />
  ...
  <table class="header" cellspacing="0" cellpadding="10" style="width: 100%;">
          <tr>
              <td colspan="10"><h5 align="center" style="font-family: Segoe Print; color:green; margin-top: 5px">{{ strtoupper($auth_data->sekolah_data->nm_yayasan_sekolah) }}</h5></td>
          </table>
          <table class="header" cellspacing="0" cellpadding="10" style="width: 100%; margin-top:-15px">
              <td  colspan="10"><h1 align="center" style="font-family: Arial Black; color:blue;">{{ strtoupper($auth_data->sekolah_data->nm_sekolah) }}</h1>
  ...
  margin-right: auto; background-color: rgb(214, 227, 188);">
          <tr>
              <td colspan=9><p align="center">Alamat :  {{ $auth_data->sekolah_data->alamat_jalan }} {{ $auth_data->sekolah_data->alamat_kelurahan }} Tlp. {{ $auth_data->sekolah_data->nomor_telp_sekolah }} – {{ $auth_data->sekolah_data->nomor_fax_sekolah }} Kecamatan {{ $auth_data->sekolah_data->alamat_kecamatan }}  Kabupaten {{ $auth_data->sekolah_data->kota->nm_kota }}, Kode Pos {{ $auth_data->sekolah_data->alamat_kodepos }} <br>E-mail : {{ $auth_data->sekolah_data->email_sekolah }} Website : {{ $auth_data->sekolah_data->website_sekolah }}</p></td>
          </tr>
      </table >
    <table class="header" cellspacing="0" cellpadding="10" style="width: 100%;">
    ...
    margin-right: auto;">Surat Keterangan Pendamping Ijazah yang dikeluarkan oleh {{ $auth_data->sekolah_data->nm_sekolah }} sebagai pelengkap ijazah yang menerangkan capaian pembelajaran dan prestasi dari pemegang ijazah selama masa studi
    @if($siswa->keterangan_kelas == 'Internasional')
    <br><i>The Diploma Supplement is issued by {{ $auth_data->sekolah_data->nm_sekolah }} Sidoarjo accompanies a higher education certificate providing a standardized description of the nature, level, context, content, and status of the studies completed by its holder</i>
  @endif
  </p>
...
margin-right: auto;">
  ...
  <tr>
    ...
    @if($siswa->keterangan_kelas == 'Internasional')
    <td style="width: 55%;"><i>Have the ability to live independently, cooperation, discipline, responsibility, and tolerance for others</i></td>
  @endif
  </tr>
  <tr>
  ...
  <tr>
    ...
    @if($siswa->keterangan_kelas == 'Internasional')
    <td style="width: 55%;"><i>Mastering communication concepts and skilled in personal communication</i></td>
    @endif
  </tr>
  <tr>
    ...
    @if($siswa->keterangan_kelas == 'Internasional')
    <td style="width: 55%;"><i>Cooperate and have social sensitivity and concern for the community and the environment</i></td>
    @endif
  </tr>
</table>

@if($prestasi->count()>0)
<h6  style="margin-top: 15px;margin-left: 120px">B.	CAPAIAN PRESTASI
