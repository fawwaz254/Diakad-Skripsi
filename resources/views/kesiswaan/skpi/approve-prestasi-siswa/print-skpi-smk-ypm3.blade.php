...
<body>
    <div class="bg">
        ...
        <table class="header" cellspacing="0" cellpadding="10" style="width: 100%; margin-top:-15px">
            <tr>
                <td colspan="10">
                    <h1 align="center" style="font-family: Arial Black; color:#6F448B;">
                        {{ strtoupper($auth_data->sekolah_data->nm_sekolah) }}
                    </h1>
                    <h3 align="center" style="margin-top:-10px;">
                        <b>
                            <span style="color:#6F448B">TERAKREDITASI : A
                                @if ($auth_data->sekolah_data->nm_singkat_sekolah == 'smkypm3taman')
                                    ( UNGGUL )
                                @endif
                            </span>
                        </b>
                        <br>NSS : 344050214028 NDS : 4305020201 NPSN : {{ $auth_data->sekolah_data->npsn_sekolah }}
                    </h3>
                </td>
            </tr>
        </table>
        ...
        <table class="header" cellspacing="0" cellpadding="10" style="width: 100%;">
            <p
                style=" border-top: 3px solid; border-bottom: 3px solid; width: 80%;margin-left: auto;margin-right: auto;">
                Surat Keterangan Pendamping Ijazah yang dikeluarkan oleh {{ $auth_data->sekolah_data->nm_sekolah }}
                sebagai pelengkap ijazah yang menerangkan capaian pembelajaran dan prestasi dari pemegang ijazah selama
                masa studi
                {{-- @if ($siswa->keterangan_kelas == 'Internasional') --}}
        ...
        @endif --}}
            ...
            {{-- @else
                <h5 style="margin-top: 40px;  margin-left: 35px"><b>II. INFORMASI TENTANG IDENTITAS PENYELENGGARA</b>
                </h5>
            @endif --}}
            ...
            <table class="bg-color break-after">
                ...
                <tr>
                    ...
                    <td> Pendidikan Formal/Sekolah Menengah Kejuruan</td>

                </tr>
                <tr>
                    <td style="width: 5%;">2.A{{ $no++ }}</td>
                    <td style="width: 40%;">Program Keahlian
                        {{-- @if ($siswa->keterangan_kelas == 'Internasional') --}}
                        <br><i>Profesional Program</i>
                        {{-- @endif --}}
                    </td>
        ...
        <table class="bg-color" style="width: 80%;border-collapse: collapse;margin-left: auto;margin-right: auto;">
            ...
            <tr>
                ...
                <td>Mampu mengintegrasikan keilmuan dan etika yang berkarakter serta memiliki moral kepribadian yang
                    baik di dalam menyelesaikan tugas</td>
                {{-- @if ($siswa->keterangan_kelas == 'Internasional') --}}
                <td style="width: 55%;"><i>Have good morals, ethics and personality in completing their duties</i></td>
                {{-- @endif --}}
            </tr>
            <tr>
                <td style="width: 5%;">3.A3</td>
                <td>Memiliki kemampuan hidup mandiri, kerja sama, tanggung jawab dan tenggang rasa sehingga mampu
                    memecahkan masalah serta beradaptasi terhadap situasi yang dihadapi</td>
                {{-- @if ($siswa->keterangan_kelas == 'Internasional') --}}
                <td style="width: 55%;"><i>Understand theories, thought, utilizing the findings of science and
                        technology in the field of science so as to be able to solve problems and adapt to the situation
                        at hand</i></td>
                {{-- @endif --}}
            </tr>
            <tr>
                <td style="width: 5%;">3.A4</td>
                <td>Terampil dalam aktivitas rumah tangga dasar, mampu merumuskan pemecahan masalah terkait dengan
                    pemahaman kompetensi bidang ilmu sosial serta dapat mengkomunikasikan dalam bentuk informasi dan
                    teknologi</td>
                {{-- @if ($siswa->keterangan_kelas == 'Internasional') --}}
                <td style="width: 55%;"><i>Ingenious in formulating problem solving related to understanding the
                        competence of science and communicate in the form of information and technology</i></td>
                {{-- @endif --}}
            </tr>
            <tr>
                <td style="width: 5%;">3.A5</td>
                <td>Terampil dalam aktivitas perawatan dasar, mampu memanfaatkan temuan ilmu pengetahuan modern dan
                    teknologi informasi untuk memperkaya wawasan di bidang ilmu sosial</td>
                {{-- @if ($siswa->keterangan_kelas == 'Internasional') --}}
                <td style="width: 55%;"><i>Able to take advantage of the findings of modern science and information
                        technology to enrich insight in the field of science</i></td>
                {{-- @endif --}}
            </tr>
            <tr>
                <td style="width: 5%;">3.A6</td>
                <td>Menguasai konsep ilmu komunikasi di bidang ilmu sosial dan terampil dalam komunikasi interpersonal
                    serta menguasai kompetensi penunjang seperti literasi dan numerasi</td>
                {{-- @if ($siswa->keterangan_kelas == 'Internasional') --}}
                <td style="width: 55%;"><i>Hold the concepts of science (Mathematics, Physics, Biology, Chemistry) as
                        well as supporting competencies such as literacy and numeracy</i>
                </td>
                {{-- @endif --}}
            </tr>
            <tr>
                <td style="width: 5%;">3.A7</td>
                <td>Mampu bekerjasama, berkolaborasi, kreatif, inovatif, memiliki kepekaan sosial, serta kepedulian
                    terhadap masyarakat dan lingkungannya</td>
                {{-- @if ($siswa->keterangan_kelas == 'Internasional') --}}
                <td style="width: 55%;"><i>Able to collaborate, be creative , innovative, have social and environment
                        awareness</i></td>
                {{-- @endif --}}
            </tr>
        </table>
        ...
        <table class="bg-color" style="width: 80%;margin-top: -20px; border-collapse: collapse; margin-left: auto;">
            <tr>
                <td style="width: 5%">3.{{ $urutan }}1</td>
                <td style="width: 40%">Bahasa Internasional
                    {{-- @if ($siswa->keterangan_kelas == 'Internasional') --}}
            ...
            <tr>
                <td>3.{{ $urutan }}2</td>
                <td>Praktik Kerja Lapangan
                    {{-- @if ($siswa->keterangan_kelas == 'Internasional') --}}
                    <br><i>On the Job Training</i>
                    {{-- @endif --}}
                </td>
                <td>Menyelesaikan Program Praktik Kerja Lapangan dengan Nilai Rata-rata 
                    {{-- @if ($siswa->keterangan_kelas == 'Internasional') --}}
                    {{-- @endif --}}
                </td>
            </tr>
            <tr>
                <td>3.{{ $urutan }}3</td>
                <td>Kegiatan Pelatihan dan Praktik
                    {{-- @if ($siswa->keterangan_kelas == 'Internasional') --}}
                    <br><i>Participation in Practical Activities</i>
                    {{-- @endif --}}
                </td>
                <td>Praktik Agama Hafalan Tahlil dan Mengkafani Jenazah dengan Kualifikasi Kompeten
                    {{-- @if ($siswa->keterangan_kelas == 'Internasional') --}}
                    <br><i>Religious Practice in Memorizing Tahlil and Shrouding the Corpse with the Competent
                        Qualification</i>
                    {{-- @endif --}}
                </td>
            </tr>
            <tr>
                <td>3.{{ $urutan }}4</td>
                <td>Partisipasi Kegiatan Penunjang
                    {{-- @if ($siswa->keterangan_kelas == 'Internasional') --}}
                    <br><i>Participation in Supporting Activities</i>
                    {{-- @endif --}}
                </td>
                <td>Perserta "Orientasi Tamu Ambalan” Pramuka SMK YPM 3 Taman Sidoarjo
                    {{-- @if ($siswa->keterangan_kelas == 'Internasional') --}}
                    <br><i>Participant of “Orientasi Tamu Ambalan” Scout of SMK YPM 3 Taman Sidoarjo</i>
                    {{-- @endif --}}
                </td>
            </tr>
            
            <p style="display:none">{{ $no = 4 }}</p>
            ...
            @if ($informasi_tambahan_praktik_kerja_lapangan->count() > 0)
                <tr>
                    <p style=" display:none">{{ $no++ }}</p>
                    <td>3.{{ $urutan }}{{ $no }}</td>
                    <td>Praktik Kerja Lapangan<br>
                        {{-- @if ($siswa->keterangan_kelas == 'Internasional') --}}
                        <i>On the Job Training</i>
                    </td>
                    {{-- @endif --}}
                    <td style="width: 55%;">
                        @foreach ($informasi_tambahan_praktik_kerja_lapangan as $r)
                            Menyelesaikan Program Praktik Kerja Lapangan dengan Nilai Rata-rata {{ $r->nm_informasi_tambahan }}
                            {{-- @if ($siswa->keterangan_kelas == 'Internasional') --}}
                            <br>
                            <i>Completed the On the Job Training with an Average Score {{ $r->nm_informasi_tambahan_eng }}</i>
                            {{-- @endif --}}
                            <br>
                        @endforeach
                    </td>
            ...
            @if ($informasi_tambahan_kompetensi_keahlian->count() > 0)
                <tr>
                    <p style=" display:none">{{ $no++ }}</p>
                    <td>3.{{ $urutan }}{{ $no }}</td>
                    <td>Uji Kompetensi Keahlian<br>
                        {{-- @if ($siswa->keterangan_kelas == 'Internasional') --}}
                        <i>Profesional Competency Assessment</i>
                    </td>
                    {{-- @endif --}}
                    <td style="width: 55%;">
                        @foreach ($informasi_tambahan_kompetensi_keahlian as $r)
                            Menyelesaikan program Uji Kompetensi Keahlian dengan Nilai {{ $r->nm_informasi_tambahan }}
                            {{-- @if ($siswa->keterangan_kelas == 'Internasional') --}}
                            <br>
                            <i>Completed the Skill Competency Test program with a Score of {{ $r->nm_informasi_tambahan_eng }}</i>
                            {{-- @endif --}}
                            <br>
                        @endforeach
                    </td>
            ...
            @if ($informasi_tambahan_kompetensi_bnsp->count() > 0)
                <tr>
                    <p style=" display:none">{{ $no++ }}</p>
                    <td>3.{{ $urutan }}{{ $no }}</td>
                    <td>Uji Kompetensi oleh BNSP<br>
                        {{-- @if ($siswa->keterangan_kelas == 'Internasional') --}}
                        <i>Competency Assessment by BNSP</i>
                    </td>
                    {{-- @endif --}}
                    <td style="width: 55%;">
                        @foreach ($informasi_tambahan_kompetensi_bnsp as $r)
                            Menyelesaikan program Uji Kompetensi oleh BNSP dengan Nilai {{ $r->nm_informasi_tambahan }}
                            {{-- @if ($siswa->keterangan_kelas == 'Internasional') --}}
                            <br>
                            <i>Completed the Competency Assessment by BNSP with a Score of {{ $r->nm_informasi_tambahan_eng }}</i>
                            {{-- @endif --}}
                            <br>
                        @endforeach
                    </td>
        ...
        <h5 style="margin-top: 80px;margin-left: 110px; margin-right: 60px"><b>IV. INFORMASI TENTANG SISTEM PENDIDIKAN DI INDONESIA SESUAI
                DENGAN KERANGKA KUALIFIKASI NASIONAL INDONESIA
                {{-- @if ($siswa->keterangan_kelas == 'Internasional') --}}
                <br><i style="margin-left: 30px">INFORMATION OF INDONESIA HIGHER EDUCATION SYSTEM AND INDONESIA
                    QUALIFICATION FRAMEWORK</i>
                {{-- @endif --}}
            </b></h5>

        <table class="bg-color" style="width: 80%; margin-top: 15px; border-collapse: collapse; margin-left: auto;">
            {{$urutan = 'A'}}
            <tr>
                <td style="width: 5%; vertical-align: top;">4.{{ $urutan }}</td>
                <td >
                </td>
                <td style="width: 55%;">
                    <p>Kerangka Kualifikasi Nasional (KKNI) adalah penjenjangan capaian yang menyetarakan luaran
                        pendidikan formal, non formal, informal, atau pengalaman kerja sesuai dengan struktur pekerjaan
                        di berbagai sektor. Jenjang kualifikasi adalah tingkat capaian pembelajaran yang disepakati
                        secara nasional, disusun berdasarkan ukuran hasil pendidikan dan/ atau pelatihan yang diperoleh
                        melalui pendidikan formal, non formal , informal, atau pengalaman kerja.</p>
                    <p>KKNI merupakan perwujudan mutu dan jati diri Banga Indonesia terkait dengan sistem pendidikan dan
                        pelatihan nasional yang dimiliki Indonesia. KKNI terdiri dari 9 (sembilan) jenjang kualifikasi,
                        dimulai dari Kualifikasi-1 sebagai Kualifikasi terendah dan Kualifikasi-9 sebagai kualifikasi
                        tertinggi.
                    </p>
                </td>
            </tr>
